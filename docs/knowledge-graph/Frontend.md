...
- `components/shared/PdfPreview.tsx` — rendering PDF su **canvas** con `react-pdf`/pdf.js, tutte le pagine impilate dentro uno `ZoomBox`. Riceve un **`Blob`** (non una URL, vedi sotto). Caricato in `React.lazy` → chunk separato (~374 kB), pdfjs non entra nel bundle principale
- `components/shared/ImagePreview.tsx` — l'immagine nello stesso `ZoomBox`, così PDF e foto si comportano uguale. L'adattamento non supera il 100%: ingrandire d'ufficio uno screenshot piccolo lo sgranerebbe
- `components/shared/ZoomBox.tsx` — riquadro ad altezza vincolata con zoom e spostamento (v0.16.0). Lo **spostamento è lo scorrimento nativo** del riquadro, non un pan a mano: si eredita l'inerzia di iOS, le barre su desktop e il multipagina. Lo **zoom cambia la dimensione di layout** del contenuto, quindi chi disegna su canvas ridisegna nitido invece di stirare una bitmap; solo durante il pinch si usa una `transform` (servono 60 fps) e si consolida a gesto finito. Ogni cambio di zoom tiene fermo il punto indicato correggendo lo scorrimento in un `useLayoutEffect`. Gesture: Ctrl/⌘+rotellina (liscia = scorre), pinch, doppio click/tap, più i pulsanti −/%/+/Adatta
...
- **Il riquadro dell'anteprima non deve prendere l'altezza dal contenuto.** `AttachmentPreviewDialog` dà al `DialogContent` un'altezza **fissa** (`h-[92dvh]`) quando dentro c'è un visualizzatore, e lo `ZoomBox` è `flex-1 min-h-0`: se l'altezza dipendesse dal contenuto, zoomando crescerebbe il riquadro invece del contenuto (visto con un'immagine piccola: riquadro da 180px che si allargava a ogni scatto di zoom). Senza visualizzatore (errore, caricamento) la modale torna alta quanto basta.
- **Il chiamante dello `ZoomBox` calcola l'adattamento, non lo `ZoomBox`**: a zoom 1 il contenuto deve starci tutto. Le pagine PDF hanno un contenitore di dimensione **esplicita** perché il canvas di react-pdf si misura da sé dentro un effect, a ridisegno finito — senza contenitore la correzione dello scorrimento lavorerebbe su numeri vecchi. Sull'adattamento si toglie qualche pixel di margine: senza, la barra di scorrimento appare e sparisce all'infinito.
