...
- **Se una funzione nuova carica risorse in modo non banale, verificala dietro nginx, non solo con `vite dev`.** In dev non ci sono né CSP né `mime.types`: due difetti dell'anteprima PDF (worker servito come `application/octet-stream` con estensione `.mjs`; XHR verso un `blob:` URL bloccato da `connect-src 'self'`) esistevano **solo in produzione**. Verifica a costo basso: `npm run build` e servire `frontend/dist` con gli stessi header della CSP di `nginx/nginx.conf`, poi aprire la pagina in modalità demo. La CSP è in [[Infrastruttura Docker]], la trappola PDF in [[Frontend]].
