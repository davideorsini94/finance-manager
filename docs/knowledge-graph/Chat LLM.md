...
- **Watchdog anti-appeso**: lo stream OpenCode viene abortito se non produce NESSUNA riga per 120s (errore riconoscibile) e c'è un fallback non-SSE (se il gateway risponde con un singolo JSON). Client-side c'è un backstop a 130s con errore parlante
